...
class MinerFanSensor(CoordinatorEntity[MinerCoordinator], SensorEntity):
    ...
    @property
    def _fan_name(self) -> str:
        """Return a display fan name based on the raw ASIC fan id."""
        fan_data = self.coordinator.data["fan_sensors"].get(self._fan_num, {})
        fan_id = fan_data.get("fan_id")
        if isinstance(fan_id, str) and fan_id.startswith("fan"):
            suffix = fan_id[3:]
            if suffix:
                return f"Fan {suffix}"
        if fan_id:
            return str(fan_id)
        return f"Fan {self._fan_num + 1}"

    @property
    def name(self) -> str | None:
        """Return name of the entity."""
        return f"{self.coordinator.config_entry.title} {self._fan_name} {self.entity_description.key}"
